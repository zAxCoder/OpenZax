...
impl Sandbox {
    pub fn new(config: SandboxConfig) -> WasmResult<Self> {
        ...
        let engine =
            Engine::new(&engine_config).map_err(|e| WasmError::Compilation(e.to_string()))?;

        info!(
            "WASM sandbox initialized with max_memory={} bytes, max_fuel={}",
    ...
    pub fn load_module_bytes(&self, wasm_bytes: &[u8]) -> WasmResult<Module> {
        debug!(
            "Loading WASM module from bytes ({} bytes)",
            wasm_bytes.len()
        );
...
impl SandboxInstance {
    ...
    pub fn check_limits(&mut self) -> WasmResult<()> {
        ...
        if memory > self.config.max_memory_bytes {
            return Err(WasmError::MemoryLimitExceeded(format!(
                "Memory usage {} exceeds limit {}",
                memory, self.config.max_memory_bytes
            )));
        }
